...
try {
    if ($action === 'create') {
        ...
        $stmt = $pdo->prepare("INSERT INTO subjects (subject_code, subject_title, units, description, is_active) VALUES (?,?,?,?,?)");
        $stmt->execute([$subject_code, $subject_title, $units, $description, $is_active]);
    ...
    if ($action === 'update') {
        ...
        $stmt = $pdo->prepare("UPDATE subjects SET subject_code=?, subject_title=?, units=?, description=?, is_active=? WHERE id=?");
        $stmt->execute([$subject_code, $subject_title, $units, $description, $is_active, $id]);
